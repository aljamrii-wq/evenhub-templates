package com.aljamri.aura.bluetooth
...
class BleManager private constructor() {
    ...
    /**
     * Disconnect from glasses: stop scan, close both left/right GATT connections,
     * reset connection state, and clear the connected device reference.
     */
    fun disconnectFromGlasses(result: MethodChannel.Result) {
        ...
        if (device == null) {
            Log.w(LOG_TAG, "disconnectFromGlasses: No connected device to disconnect")
            result.success("No device connected.")
            return
        }
        // Disconnect and close GATT for both left and right peripherals
        val leftGatt = device.leftDevice?.gatt
        val rightGatt = device.rightDevice?.gatt
        try {
            leftGatt?.disconnect()
            leftGatt?.close()
            device.leftDevice?.gatt = null
            device.leftDevice?.isConnect = false
            Log.d(LOG_TAG, "disconnectFromGlasses: Left GATT disconnected and closed")
        } catch (e: Exception) {
            Log.e(LOG_TAG, "disconnectFromGlasses: Error disconnecting left GATT", e)
        }
        try {
            rightGatt?.disconnect()
            rightGatt?.close()
            device.rightDevice?.gatt = null
            device.rightDevice?.isConnect = false
            Log.d(LOG_TAG, "disconnectFromGlasses: Right GATT disconnected and closed")
        } catch (e: Exception) {
            Log.e(LOG_TAG, "disconnectFromGlasses: Error disconnecting right GATT", e)
        }
        // Clear connected device reference
        connectedDevice = null
        Log.i(LOG_TAG, "disconnectFromGlasses: All GATT resources released")
