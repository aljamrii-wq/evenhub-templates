...
`disconnectFromGlasses` stops any active scan, calls `disconnect()` + `close()` on both left/right GATT connections, resets connection state, and clears the connected device reference.
