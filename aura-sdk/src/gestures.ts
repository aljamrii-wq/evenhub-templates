...
export class GestureEngine {
  ...
  process(x: number, y: number, z: number): GestureEvent {
    ...
    if (lookDir !== 'unknown') {
      return this.emit(lookDir as GestureEvent['type'], 0.8);
    }
  ...
  private detectLook(frames: IMUFrame[]): string {
  ...
  private emit(type: GestureEvent['type'], confidence: number): GestureEvent {
